...
class ShowThread extends Thread{
	...
	/*
	 ...
	 Thread 클래스의 run() 메서드를 오버라이딩한 것으로 해당 메서드는
	...
	public void run()
	{
		for(int i=1 ; i<=100 ; i++) {
			...
			try {
				/*
				 Thread클래스의 static 메서드로 실행의 흐름을 주어진 시간만큼
				 block 상태로 전환시켜 준다.
				 1000분의 1초 단위로 기술하게 되므로 아래는 0.01초 만큼
				 block 시키게 된다. 즉 1000이 1초이다.
				 */
				sleep(10);
			}
...
public class Ex01ThreadStart
{
	public static void main(String[] args)
	{
		...
		/*
		 run()메소드를 통한 호출은 단순한 실행만 될 뿐 쓰레드가
		 생성되지 않는다. 아래의 경우 첫번째 쓰레드 객체의 모든 작업이
		 완료된 후 두번째 쓰레드가 동작한다.
		 */
//		st1.run();
//		st2.run();
