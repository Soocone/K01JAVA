...
class Police{
	...
	public Police(int bNum, int hCuff) {
		...
		/*
		 권총의 보유여부 초기화
		 	...
		 	참조변수가 null값이란 것은 참조할 객체가 없는 것을 의미한다.
		 	즉 Heap영역에 생성된 객체가 없다는 말이다.
		 */
		if(bNum <=0) {
			gun = null;
		}
	...
	public void shut() {
		if(gun == null) {
			...
			System.out.println("보유한 권총 없음. ㅋㅋ");
		}
		else {
			// 보유하고 있으므로 Gun 클래스의 멤버메서드를 호출하여 실행한다.
			gun.shutGun(); 
		}
	}
}
