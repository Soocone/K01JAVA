...
/*
 ...
 오버로딩: 메소드명은 같으나 매개변수의 갯수, 타입, 순서가
 	다른 경우를 말한다. 즉 서로 다른 메서드이므로 상속받은
 	하위클래스에서는 오버로딩된 모든 메서드가 포함된다.
...
class C extends B{
	...
	/*
	 ...
	 오버로딩된 메서드는 서로 다르므로 3개 모두 존재하게 된다.
	 */
}
...
public class E07RideAndLoad01
{
	public static void main(String[] args)
	{
		...
		C refAddr1 = new C(); //가능
		B refAddr2 = refAddr1; //B타입 => C타입 참조: 가능
		A refAddr3 = refAddr1; //A타입 => C타입 참조: 가능
		
		/*
		 참조변수 refId1이 C 객체를 참조하고 있으므로 마치 refId1이 C타입이라고
		 착각할 수 있으나, 처음부터 refId1은 A타입의 참조변수이다.
		 자식으로 부모를 참조하는 것은 불가능하므로 아래 2개의 문장은
		 에러가 발생한다. 
		 */
		A refId1 = new C(); //가능
		//B refId2 = refId1; //B타입 => A타입 참조: 불가능(에러 발생)
		//C refId3 = refId1; //C타입 => A타입 참조: 불가능(에러 발생)
