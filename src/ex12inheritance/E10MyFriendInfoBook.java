...
class Friend{
	...
	/*
	 간략 정보를 출력하는 용도의 멤버메서드로 실행부가 없는 상태로 
	 정의한다. 해당 프로그램에서는 오버라이딩의 목적으로만 사용된다.
	 */
...
class HighFriend extends Friend{
	...
	/*
	 자식에서 오버라이딩하여 재정의한 메서드로 고딩친구의 간략한
	 정보를 출력한다.
	 */
	@Override
	public void showBasicInfo()
...
class FriendInfoHandler{
	...
	private Friend[] myFriends;  //객체배열. 객체를 저장할 수 있는 배열.
	//실제 저장된 연락처 갯수
	...
	/*
	 ...
	 3. 정보 출력시 배열에 입력된 객체수만큼 반복하면서 각 요소를 통해
	 	멤버메서드를 호출한다.
	 4. 이때 오버라이딩된 메서드를 통해 항상 해당 객체의 정보를 
	 	출력할 수 있다. 참조변수의 타입에 영향을 받지 않고 항상 오버라이딩된
	 	최하위의 메소드가 호출되기 때문이다.
	 */
	//전체정보 출력용 메서드
	public void showAllData() {
	...
	public void searchInfo() {
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			/*
			 검색을 위해 입력한 이름과 각 인덱스에 저장된 객체의 name 변수의
			 비교를 통한 문자열검색을 진행한다.
			 */
	...
	public void deleteInfo() {
		...
		/*
		 ...
		 배열은 인덱스(0부터시작)이므로 초기값은 -1로 설정한다.
		 */
		int deleteIndex = -1; 
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			...
			if(deleteName.compareTo(myFriends[i].name)==0) {
				...
				//전체카운트를 1 차감한다.
				numOfFriends--;
				//하나의 객체를 삭제했다면 즉시 for문 탈출(뒤에 배열은 더 볼필요 X)
				break;
			}
		}
		if(deleteIndex== -1) {
	...
	public static void main(String[] args)
	{
		...
		while(true) {
			...
			case 1: case 2:
				...
				break; //break를 만나면 가장 가까운 (switch문)을 탈출한다
			case 3:
				//전체정보 출력
