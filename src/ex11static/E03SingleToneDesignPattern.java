...
/*
 싱글톤 디자인 패턴
 	...
 	특정 갯수를 정해놓고 그 이상 생성되지 않게 하기 위할때 주로 씀 
 작성방법
 	1. 생성자의 접근지정자를 private으로 정의한다.
 	2. 클래스 외부에서는 생성자를 통해 접근할 수 없으므로
 		new를 통해 객체생성은 할 수 없다.
 	3. 클래스 내부에 정적메서드로 해당 클래스의 객체를 반환하도록 정의한다.
 		=> getInstance()   static으로 객체만들었기때문에 메서드도 스태틱으로 생성.
 	4. 위 방법을 통해 객체를 생성하면 메모리에 딱 하나의 객체만 만들어진다.
 */
...
class SingleTone {
	...
	/*
	 정적메서드로 선언된 getInstance() 메서드에서 해당 객체를 외부로
	 반환한다.
	 <<static으로 준비되어 있는 정적변수를 (static메서드를 통해) 호출한다.>>
	*/
...
public class E03SingleToneDesignPattern
{
	public static void main(String[] args)
	{
		...
		//SingleTone st1 = new SingleTone();  -> new 사용 불가
