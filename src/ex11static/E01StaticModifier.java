...
- main() 메서드가 실행되기 전에 이미 초기화되므로 main() 메서드 실행시
 	별도의 선언없이  변수를 사용할 수 있다.
- Static은 객체생성없이 편하게 쓰려고 사용하는 거임 !!!!!!!!!!!!!
...
class MyStatic{
	/*
	 멤버변수] 인스턴스형 멤버변수(일반적인)는 반드시 클래스의 객체를 생성한 후
	 	참조변수를 통해 접근해야 한다.
	 */
	int instanceVar;
	/*
	 정적멤버변수] 객체를 생성할 필요 없이 클래스명으로 바로 접근할 수 있다.
	...
	void instanceMethod() {
		...
		System.out.println("stacicVar="+ staticVar);
		staticMethod();
	}
...
public class E01StaticModifier
{
	...
	{
		/*
		 일반적인 멤버변수에 접근하느 방법: MyStatic 클래스의 객체를
		 	생성한 후 참조변수를 통해 접근한다.
		 */
