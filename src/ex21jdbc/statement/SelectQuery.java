...
/*
 ResultSet 클래스
 	...
 	오라클의 자료형이
 		...
 		date타입: getDate()
 	-인자는 select절의 순서대로 인덱스(1부터 시작)를 사용하거나
 	컬럼명을 사용할 수 있다.
 */
public class SelectQuery extends ConnectDB
{
	...
	void execute()
	{
		try {
			...
			while(rs.next()) {
				String id = rs.getString(1); //id컬럼.  1=> 컬럼명 대신 인덱스를 사용함.
				String pw = rs.getString("pass"); //컬럼명 사용하는 것이 나중에 수정/보완 용이
				String name = rs.getString("name");
				
				/*
				 오라클의 date타입을 getDate()로 추출하면 
				 0000-00-00 형태로 출력된다.
				 이 경우 date형 자료가 되므로 java.sql.Date 클래스의 
				 참조변수로 저장해야 한다.
				 */
