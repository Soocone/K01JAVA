...
class Friend implements Serializable{
	...
	public Friend(ArrayList<Friend> lists)
	{
...
class FriendInfoHandler{
	...
	public void addFriend(int choice) {
		...
		if(choice ==1) { //고딩친구 입력 후 추가
			...
			//고딩친구 객체를 만든후 컬렉션에 add한다. 인덱싱은 필요 없다.
	...
	//전체정보 출력용 메서드(3가지 방법. for, 확장for, iterator)
	/*
	 일반 for문을 통해 컬렉션에 접근한다. 이때는 인덱스를 사용하므로 
	...
	public void showSimpleData() {
		for(Friend fr : lists) {  //Friend 객체가 저장된 lists 리스트를 반복하겠다.
			fr.showBasicInfo();
		}
	...
	/*
	 연습문제] 친구정보를 파일로 직렬화 하기위한 메서드를 정의하시오.
	 	프로그램 종료시 컬렉션에 저장된 모든 친구 객체를 파일로 저장해야 한다.
...
//							new FileOutputStream("src/ex20io/friend_info.obj"));
//			
//			out.writeObject(new Friend(lists));
//			
//			
//			out.close();
//			
