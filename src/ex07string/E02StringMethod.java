...
public class E02StringMethod
{
	...
	public static void main(String[] args)
	{
		...
		//위에서 찾은 인덱스를 통해 문자를 가져와 조건을 확인한다.
		...
		/*
		 11] lastIndexOf()
		 	...
		 	찾는것이 다르다.
		 	(확장자같은걸 .으로 구분할때 맨 마지막 .을 기준으로 해야하기 때문에
		 	이런 경우에 사용)
		 */
		...
		/*
		 14] substring()
		 	: 시작인덱스와 끝 인덱스 사이의 문자열을 잘라서	반환해준다.
		 	
		 	사용1: substring(인덱스) => 이 경우 인덱스부터 끝까지를 잘라낸다.
		 	사용2: substring(시작인덱스, 끝인덱스) => 인덱스에 해당하는 
		 		부분만 잘라서 반환한다.
		 */
		...
		/*
		 15] codePointAt(인덱스)
		 	: 문자열의 특정 인덱스의 문자를 찾은후 아스키코드(유니코드)를 반환한다.
		 */
