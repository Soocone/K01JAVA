...
class ChildProperty_t{
	
	//멤버변수
	int bead;  //어린이가 보유한 구슬의 갯수
	
	//생성자: 객체생성시 구슬의 갯수를 전달받아 초기화
	public ChildProperty_t(int bead)
	{
		this.bead = bead;
	}
	...
	/*
	 해당 멤버메서드의 호출의 주체가 되는 객체가 beadCnt만큼
	 획득하고 매개변수로 전달되는 child객체는 구슬을 잃게 된다.
	 */
	//어디 대입되거나 프린트문에 쓰인게 아니라서 반환값이 없는 void타입!!
	void obtainBead(ChildProperty_t child, int beadCnt) {
		this.bead += beadCnt;
		child.bead -=beadCnt;
	}
}
...
public class QuMarbles_t
{
	...
	{        	
		ChildProperty_t child1 = new ChildProperty_t(20);
		ChildProperty_t child2 = new ChildProperty_t(15);
