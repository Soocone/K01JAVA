...
public class E05DateCalendar1
{
	...
	/*
	 Calendar 클래스를 통해 얻은 객체로 현재 시간을 초로 반환받은 후
	 날짜를 원하는 포맷의 문자열로 반환한다.
	 */
	public static String myDateFormat(Calendar c, String formatStr) {
		Date date = c.getTime(); //getTime(): 1970년 1월 1일부터 지금까지의 시간을 초로 반환
	...
	public static String[] myDiffDate() throws ParseException{
		...
		/*
		 ...
		 즉 1000으로 난눠서 뒤의 3자리를 제거하면 순수한 초가 반환된다.
		 */
		long diffSec = (formatFuture.getTime() - formatToday.getTime()) / 1000;  //초
		long diffMin = (formatFuture.getTime() - formatToday.getTime()) / 60000;  //60초(1분)
		long diffHor = (formatFuture.getTime() - formatToday.getTime()) / 3600000; //3600초(1시간)
		long diffDays = diffSec / (24*60*60);  //하루는 86400초이므로 나눠서 일수를 계산.
	...
	{
		//Calendar 클래스는 대표적인 싱글톤이므로 new로 생성하지 않는다.
		//싱글톤이니까 getInstance() 유틸메서드 사용
		Calendar calendar = Calendar.getInstance();
		
		//여러가지 서식문자 활용하기
		System.out.println("오늘 날짜:"+ myDateFormat(calendar, "yyyy-MM-dd"));
		System.out.println("현재 시간:"+ myDateFormat(calendar, "HH:mm:ss"));
		System.out.println(myDateFormat(calendar, "오늘은 yyyy년의 w주차, D번째 날"));
		System.out.println(myDateFormat(calendar, "오늘은 M월의 W번째 주, d번째 날, F번째 E요일"));
		...
		System.out.println("내일 날짜:"+ dateStr5);
